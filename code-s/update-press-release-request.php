    ...
    <div class="col-sm-12">
        <div class="box box-primary">
            <div class="box-header with-border">
                <div class="left">
                    <h3 class="box-title"><?php //echo trans("update_press_release"); ?>Update Press Request</h3>
                </div>
                 <?php 
            ...
            <!-- form start -->
    <?php echo form_open_multipart('press_realease_controller/update_request_press_release_value_post'); ?>
          
    <div class="box-body">
       <?php if(!empty($view_press_release[0])){?>
        <input type = "hidden" name="id" value="<?php echo $view_press_release[0]->press_release_id; ?>" />
...
foreach($view_press_release_infographic as $items){?> 
       <div class="row">
          ...
          <div class="col-sm-8">
            <input type="hidden" name="infographic_path[data][title]" value="<?php echo $items->caption;?>">
            <input type="hidden" name="infographic_path[data][path]" value="<?php echo $items->media_path;?>">
            <input type="hidden" name="infographic_path[data][media_size]" value="<?php echo $items->media_size;?>">
            <input type="hidden" name="infographic_path[data][media_format]" value="<?php echo $items->media_format;?>">
             <input type="hidden" name="video_path[data2][press_release_row_id]" value="<?php echo $items->press_release_row_id;?>">
        <input type="hidden" name="video_path[data2][press_release_media_id]" value="<?php echo $items->press_release_media_id;?>">
...
foreach($view_press_release_image as $items){?>
         ...
         <input type="hidden" name="image_path[data1][title]" value="<?php echo $items->caption;?>">
            <input type="hidden" name="image_path[data1][path]" value="<?php echo $items->media_path;?>">
            <input type="hidden" name="image_path[data1][media_size]" value="<?php echo $items->media_size;?>">
            <input type="hidden" name="image_path[data1][media_format]" value="<?php echo $items->media_format;?>">
             <input type="hidden" name="video_path[data2][press_release_row_id]" value="<?php echo $items->press_release_row_id;?>">
        <input type="hidden" name="video_path[data2][press_release_media_id]" value="<?php echo $items->press_release_media_id;?>">

          <div class="col-sm-8">
             <input type="text" id="caption-image-texdatat" class="form-control" name="caption-image-text[]" placeholder="Caption" value="<?php echo $items->caption;?>">
          </div><a href="javascript:void(0);" onclick="remove_media('<?php echo $items->id.','.$items->release_id.','.$items->media_type.','.$items->lang_id.','.$items->media_format.','.$items->media_size.','.$items->media_path.','.$items->caption.','.$items->status.','.$items->schedule_for_publish.','.$items->created_at.','.$items->created_by;?>');" class="remove_button"><img src="<?php echo base_url(); ?>/assets/img/delete-image.png" width="35px" height="35px"/></a>
       </div>  
<?php 
}
...
foreach($view_press_release_video as $items){?>
 <div class="row">
           <div class="col-sm-3">
            ...
            <input type="hidden" name="video_path[data2][title]" value="<?php echo $items->caption;?>">
            <input type="hidden" name="video_path[data2][path]" value="<?php echo $items->media_path;?>">
            <input type="hidden" name="video_path[data2][media_size]" value="<?php echo $items->media_size;?>">
            <input type="hidden" name="video_path[data2][media_format]" value="<?php echo $items->media_format;?>">
        <input type="hidden" name="video_path[data2][press_release_row_id]" value="<?php echo $items->press_release_row_id;?>">
        <input type="hidden" name="video_path[data2][press_release_media_id]" value="<?php echo $items->press_release_media_id;?>">
             ...
             <div class="form-group">
              ...
              <?php }else  if($this->auth_user->role == 'hq admin' && $this->auth_user->pro_category_id = $pro['0']['id'] && $this->auth_user->permission = 2)
                ...
                <?php 
                     ...
                     if($view_press_release[0]->status == 2){?>
                  <div class="col-md-2 col-sm-2 col-xs-12 col-option">
                            <input type="radio" id="rb_is_active_2" name="status" value="3" onclick="hideDiv()" <?php if($view_press_release[0]->status == 3){ echo "checked=checked";}  ?>>
                            <label for="rb_is_active_2" class="cursor-pointer"><?php echo trans('publish'); ?></label>
                        </div>
                    ...
                    <?php }else{?>
                            ...
                            <div class="col-md-2 col-sm-2 col-xs-12 col-option">
                            <input type="radio" id="rb_is_active_2" name="status" value="7" onclick="hideDiv()" <?php if($view_press_release[0]->status == 7){ echo "checked=checked";}  ?>>
                            <label for="rb_is_active_2" class="cursor-pointer"><?php echo trans('withdraw'); ?></label>
                        </div>
