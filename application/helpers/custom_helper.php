...
//get Added language id
if (!function_exists('get_add_id')) {
    function get_add_id($id)
    {  
        $ci =& get_instance();
        $sql = "SELECT lang_id FROM  sainik_pratika_master WHERE translated_id=0 and  id=$id  or translated_id=$id group by translated_id,lang_id order by lang_id ASC ";
...
//get Added language name
if (!function_exists('get_add_lang')) {
    function get_add_lang($id)
    {
       $ci =& get_instance();
...
//Update o translated_id
if (!function_exists('update_translated_id')) {
    function update_translated_id($id)
    {
       $ci =& get_instance();
       $sql2 = "SELECT * FROM languages WHERE 1 ";
       $query2 = $ci->db->query($sql2);
        $counlang = $query2->num_rows();
       $sql = "SELECT * FROM sainik_pratika_master WHERE translated_id!= 0 group by lang_id ";
       $query = $ci->db->query($sql);
        $coundata = $query->num_rows();
       if($counlang==$coundata){
           
            $ci->db->where('id', $id);
            $ci->db->where('translated_id', 0);
            $ci->db->where('id', $id);
            $ci->db->update('sainik_pratika_master', array('translated_id' => $id));
...
if (!function_exists('get_month_last_day')) {
    function get_month_last_day($month,$year)
    {
...
if (!function_exists('get_logo2')) {
    function get_logo2($settings)
    {
        if (!empty($settings)) {
            if (!empty($settings->logo2) && file_exists(FCPATH . $settings->logo2)) {
                return base_url() . $settings->logo2;
            } else {
                return base_url() . "assets/img/logo.svg";
            }
        } else {
...
//get favicon
if (!function_exists('home_popup_image')) {
    function home_popup_image($settings)
    {
        if (!empty($settings)) {
            if (!function_exists('get_site_mod')) {
                exit();
            }
            if (!empty($settings->favicon) && file_exists(FCPATH . $settings->home_popup_image)) {
                return base_url() . $settings->home_popup_image;
            } else {
                return base_url() . "assets/img/home_popup_image.jpeg";
            }
        } else {
            return base_url() . "assets/img/home_popup_image.jpeg";
        }
...
if (!function_exists('get_circular_category')) {
    function get_circular_category($id)
    {
        $ci =& get_instance();
        return $ci->circular_model->get_circular_categroy($id);
    }
}

if (!function_exists('get_document_category')) {
    function get_document_category($id)
    {
        $ci =& get_instance();
        return $ci->document_model->get_document_categroy($id);
    }
}

if (!function_exists('get_download_category')) {
    function get_download_category($id)
    {
        $ci =& get_instance();
        return $ci->download_model->get_download_categroy($id);
    }
}



if (!function_exists('get_audio_album')) {
    function get_audio_album($id)
    {
      
        $ci =& get_instance();
        return $ci->gallery_category_model->get_audio_album($id);
    }
}

if (!function_exists('get_video_album')) {
    function get_video_album($id)
    {
      
        $ci =& get_instance();
        return $ci->gallery_category_model->get_video_album($id);
    }
}
...
if (!function_exists('get_pro_category')) {
    function get_pro_category($id)
    {
        $ci =& get_instance();
        return $ci->category_model->get_pro_category($id);
    }
}


if (!function_exists('get_sainikpatrika_category')) {
    function get_sainikpatrika_category($id)
    {
        $ci =& get_instance();
        return $ci->SainikPratika_model->get_sainikpatrika_category($id);
    }
}

if (!function_exists('get_infographic_category')) {
    function get_infographic_category($id)
    {
        $ci =& get_instance();
        return $ci->gallery_category_model->get_infographic_category_edit($id);
    }
}


//get audio category
if (!function_exists('get_audio_category')) {
    function get_audio_category($id)
    {
        $ci =& get_instance();
        return $ci->gallery_category_model->get_audio_category($id);
    }
}

if (!function_exists('regional_pro_name')) {
    function regional_pro_name($id)
    {
        $ci =& get_instance();
        return $ci->gallery_category_model->regional_pro_name($id);
    }
}
...
if (!function_exists('get_video_category')) {
    ...
    {
        $ci =& get_instance();
        return $ci->gallery_category_model->get_video_category($id);
    }
}
...
function send_email($data, $email)
{
    ...
    //   echo "<pre>"; print_r($data); die;
        $CI =& get_instance();
        $CI->load->library('Phpmailer_lip');
        // $this->load->library('Phpmailer_lip');        
        $mail = $CI->phpmailer_lip->load();
        // $mail->SMTPDebug = 1;  
        $mail->ClearAddresses();
...
if (!function_exists('get_copyright')) {
    function get_copyright(){
       $lang_id = !empty($_SESSION['lang_id'])?$_SESSION['lang_id']:1; //$this->session->userdata('lang_id');
      //print_r($lang_id);
       $ci =& get_instance();
       $ci->db->select('site_title,site_description,keywords,application_name,twitter_url   ,facebook_url,instagram_url ,pinterest_url  ,linkedin_url   ,vk_url ,telegram_url   ,youtube_url    ,optional_url_button_name   ,about_footer   ,contact_text   ,contact_address    ,contact_email  ,contact_phone  ,copyright  ,cookies_warning    ,cookies_warning_text');
...
/// All title short content
if (!function_exists('get_all_title_short')) {
    function get_all_title_short($lable){
       $lang_id = !empty($_SESSION['lang_id'])?$_SESSION['lang_id']:1; //$this->session->userdata('lang_id');
      //print_r($lang_id);
     
        $ci =& get_instance();
        $ci->db->select('translation');
        $ci->db->from('language_translations');
        $ci->db->where('lang_id',$lang_id);
        $ci->db->where('label',$lable);
        $query = $ci->db->get();
        $rowcount = $query->num_rows();
        // print_r($query->row());
        if(!empty($rowcount)){
  ...
  if (!function_exists('total_views')) {
    function total_views($page_id = null){
        ...
        if($page_id === null) {
            ...
            $ci =& get_instance();
            $ci->db->select('count(id) as views_count');
            $ci->db->from('pageviews');
            $query = $ci->db->get();
            $rowcount = $query->num_rows();
            // print_r($query->row());
            if(!empty($rowcount)){
    ...
    if (!function_exists('is_unique_view')) {
        function is_unique_view($ip_address , $page_id, $user_agent){
            //$query = "SELECT * FROM page_views WHERE ip_address   ='$ip_address   ' AND page_id='$page_id'";
            $ci =& get_instance();
            $ci->db->select('*');
            $ci->db->from('pageviews');
            $ci->db->where('page_id',$page_id);
    ...
    if (!function_exists('add_view')) {
        function add_view($ip_address, $page_id, $user_agent){
            //echo is_unique_view($ip_address, $page_id);
            if(is_unique_view($ip_address, $page_id, $user_agent) === true){
                ...
                $pattern2 = "/css/i";
                $pattern3 = "/js/i";
                if( preg_match_all($pattern3, $page_id) == true ||  preg_match_all($pattern1, $page_id) == true || preg_match_all($pattern2, $page_id) == true || preg_match($pattern, $page_id) == true){
                    ...
                    'created_at'=>$date
                     ...
                     $ci->db->insert('pageviews',$data);
                }
                
            }else{
                $ci =& get_instance();
                $sql = "UPDATE pageviews SET views_count = views_count + 1 WHERE page_id='$page_id'";
                 $ci->db->query($sql);
