    ...
    public function get_latest_media_invites_list($lang_id,$pro_id,$title,$invite_from_date,$invite_to_date,$limit,$start)
     {
        ...
        $this->db->select('id,lang_id,title,venue_event,pro_category,regional_pro_id,name,mobile,date_of_event,reporting_time,remark,invitees,keywords,description,created_at,updated_at');
         $this->db->from('media');
       ...
       if(!empty($invite_from_date) && !empty($invite_to_date)){
         $this->db->where('date_of_event BETWEEN "'. date('Y-m-d 00:00:00', strtotime($invite_from_date)). '" and "'. date('Y-m-d 23:59:59', strtotime($invite_to_date)).'"');
       } 
       if(!empty($title)){
         $this->db->group_start();
         $this->db->like('title',$title,'both',false);
         $this->db->or_like('keywords',$title,'both',false);
         $this->db->or_like('description',$title,'both',false);  
         $this->db->or_like('remark',$title,'both',false); 
         $this->db->group_end();    
       }
