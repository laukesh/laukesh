...
class Home_model extends CI_Model
{
    ...
    public function photo($lang_id)
    {
       ...
       // $query = $this->db->query("SELECT * FROM gallery  where lang_id=$lang_id ORDER BY id DESC LIMIT 10");
        $query = $this->db->query("SELECT a.media_path,b.press_release_title FROM `tbl_press_release_media` a left join press_release b on a.release_id = b.id where a.status = 3 and a.lang_id = $lang_id and a.media_type ='press_release_image' group by a.release_id order by a.release_id desc LIMIT 10;");
        return $query->result();
    }

     public function latest_image($lang_id)
    {
       
        $query = $this->db->query("SELECT * FROM `tbl_press_release_media`  where status = 3 and lang_id = $lang_id and media_type ='press_release_image' group by release_id order by release_id desc LIMIT 10;");
        return $query->result();
    }

    public function get_settings_uploaded_date()
    {
        $query = $this->db->query("SELECT * FROM visual_settings WHERE id = 1");
        return $query->result();
    }

    public function get_settings_home_popup()
    {
        $query = $this->db->query("SELECT * FROM visual_settings WHERE id = 1 and status = 1");
        return $query->result();
    }
 ...
 $query = $this->db->query("SELECT * FROM audios where status = 1 and lang_id = $lang_id ORDER BY updated_at  DESC LIMIT 5");
    ...
    public function get_lang_bradcrumb($lang_id2)
    { 
        $sql = "SELECT * FROM languages where id = ".$lang_id2."";
        $query = $this->db->query($sql);
        return $query->result();
    }
    ...
    public function youtube_video($lang_id)
    {
        $query = $this->db->query("SELECT * FROM video  where is_album_cover = 1 and status = 1 and lang_id = $lang_id ORDER BY id DESC LIMIT 10");
        //echo $this->db->last_query();
        return $query->result();
    }
    ...
    {
        $query = $this->db->query("SELECT * FROM sainik_pratika_master where lang_id = $lang_id ORDER BY year DESC,month DESC,biweek_no DESC LIMIT 1");
        //echo $this->db->last_query();
        return $query->result();
    }
  ...
  public function sainik_samachar_list_last12_months($limit,$start,$lang_id,$visibility)
    {    

        $year           = date('Y-m-01');
      $month           = date('m'); 
      $yearold = date('Y-m-01', strtotime('-1 year'));   
        // echo $yearold;
     //die();
        ...
        $this->db->select('id,lang_id,title,year,month,biweek_no,visibility,volume');
        $this->db->where('str_to_date(CONCAT(year, "-", month,"-01"),"%Y-%m-%d") BETWEEN "'.$yearold.'" and "'.$year.'"');
        $this->db->from('sainik_pratika_master');
        if(!empty($lang_id)){
         $this->db->where('lang_id',$lang_id);
        }
      ...
      public function pru_cate($lang_id)
    {
        ...
        if($lang_id==1){      
        $query = $this->db->query("SELECT id,name,lang_id FROM tbl_pru_category  where lang_id ='$lang_id' order by CASE 
            WHEN name LIKE '%PRO AIR%' THEN 1
            WHEN name LIKE '%PRO ARM%' THEN 2
            WHEN name LIKE '%PRO DEF%' THEN 3
            WHEN name LIKE '%PRO NAV%' THEN 4
            ELSE name 
            END; ");
        }
        else{
    ...
    {   $time           = strtotime('-14 days');
        $gallery_from_date           = date('Y-m-d',(strtotime ( '-2 year' , strtotime (date('Y-m-d')) ) ));
  ...
  public function get_paginated_last12months_sainik_samachar_count($lang_id)
  {
      $year           = date('Y-m-01');
      $month           = date('m'); 
      $yearold = date('Y-m-01', strtotime('-1 year'));      
    ...
    $this->db->select('COUNT(id) AS count, CONCAT(year, "-", month)');      
    $this->db->where('str_to_date(CONCAT(year, "-", month,"-01"),"%Y-%m-%d") BETWEEN "'.$yearold.'" and "'.$year.'"');
