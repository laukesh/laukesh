...
class Gallery_controller extends Admin_Core_Controller
{
    ...
    public function update_gallery_image($id)
    {
       ...
       // $data['categories'] = $this->gallery_category_model->get_categories_by_album_image($data['image']->album_id);
    ...
    public function update_gallery_image_post()
    {
        ...
        if ($this->form_validation->run() === false) {
            ...
            if ($this->gallery_model->update($id)) {
                $this->session->set_flashdata('success', trans("image") . " " . trans("msg_suc_img_updated"));
    ...
    public function update_gallery_album_post()
    {
        ...
        if ($this->form_validation->run() === false) {
            ...
            if ($this->gallery_category_model->update_album($id)) {
                $this->session->set_flashdata('success', trans("category") . " " . trans("msg_suc_img_updated"));
                redirect(admin_url() . 'gallery-albums');
            } else {
                $this->session->set_flashdata('error', trans("msg_error"));
                redirect($this->agent->referrer());
    ...
    public function gallery_video_by_lang()
    {
        $lang_id = $this->input->post('lang_id', true);
        if (!empty($lang_id)):
            $albums = $this->gallery_category_model->get_video_albums_by_lang($lang_id);
            foreach ($albums as $item) {
                echo '<option value="' . $item->id . '">' . $item->name . '</option>';
            }
        endif;
     ...
     public function gallery_audio_by_lang()
    {
        $lang_id = $this->input->post('lang_id', true);
        if (!empty($lang_id)):
            $albums = $this->gallery_category_model->get_audio_albums_by_lang($lang_id);
            foreach ($albums as $item) {
                echo '<option value="' . $item->id . '">' . $item->name . '</option>';
            }
        endif;
