...
class Category_controller extends Admin_Core_Controller
{
    ...
    public function audio_categories()
    {
        //echo '===';die();
        check_permission('audio');
        $data['title'] = trans("audio_categories");
        $data['categories'] = $this->gallery_category_model->get_all_audio_categories();
        $data['panel_settings'] = panel_settings();
        $data['albums'] = $this->gallery_category_model->get_audio_albums_by_lang($this->selected_lang->id);
        $data['lang_search_column'] = 3;

        $this->load->view('admin/includes/_header', $data);
        $this->load->view('admin/audio/audio_categories', $data);
        $this->load->view('admin/includes/_footer');
    }
    ...
    {
        //echo '===';die();
        check_permission('video');
        $data['title'] = trans("audio_categories");
        $data['categories'] = $this->gallery_category_model->get_all_video_categories();
        $data['panel_settings'] = panel_settings();
        $data['video_albums'] = $this->gallery_category_model->get_video_albums_by_lang($this->selected_lang->id);
        $data['lang_search_column'] = 3;

        $this->load->view('admin/includes/_header', $data);
        $this->load->view('admin/video/video_categories', $data);
        $this->load->view('admin/includes/_footer');
    }
    ...
    public function add_audio_category_post()
    {
        check_permission('audio');
        //validate inputs
        $this->form_validation->set_rules('name', trans("category_name"), 'required|xss_clean|max_length[200]');
        ...
        if ($this->form_validation->run() === false) {
            $this->session->set_flashdata('errors_form', validation_errors());
        $this->session->set_flashdata('form_data', $this->gallery_category_model->input_values_cat());
            redirect($this->agent->referrer());
        } else {
            if ($this->gallery_category_model->add_audio_cat()) {
                $this->session->set_flashdata('success_form', trans("category") . " " . trans("msg_suc_added"));
                redirect($this->agent->referrer());
            } else {
                $this->session->set_flashdata('form_data', $this->gallery_category_model->input_values_cat());
     ...
     public function add_video_category_post()
    {
        check_permission('audio');
        //validate inputs
        $this->form_validation->set_rules('name', trans("category_name"), 'required|xss_clean|max_length[200]');
        ...
        if ($this->form_validation->run() === false) {
            $this->session->set_flashdata('errors_form', validation_errors());
        $this->session->set_flashdata('form_data', $this->gallery_category_model->input_values_cat_video());
            redirect($this->agent->referrer());
        } else {
            if ($this->gallery_category_model->add_audio_cat_video()) {
                $this->session->set_flashdata('success_form', trans("category") . " " . trans("msg_suc_added"));
                redirect($this->agent->referrer());
            } else {
        $this->session->set_flashdata('form_data', $this->gallery_category_model->input_values_cat_video());
    ...
    public function update_gallery_category($id)
    {
     ...
     public function update_audio_category($id)
    {

        check_permission('audio');
        $data['title'] = trans("update_category");
        //get category
        $data['category'] = $this->gallery_category_model->get_audio_category($id);
        //echo '<pre>';print_r($data['category']);die();
        if (empty($data['category'])) {
            redirect($this->agent->referrer());
        }
        $data['albums'] = $this->gallery_category_model->get_audio_albums_by_lang($data['category']->lang_id);

        $this->load->view('admin/includes/_header', $data);
        $this->load->view('admin/audio/update_audio_category', $data);
        $this->load->view('admin/includes/_footer');
    }


     public function update_video_category($id)
    {

        check_permission('video');
        $data['title'] = trans("update_category");
        //get category
        $data['category'] = $this->gallery_category_model->get_video_category($id);
        //echo '<pre>';print_r($data['category']);die();
        if (empty($data['category'])) {
            redirect($this->agent->referrer());
        }
        $data['video_albums'] = $this->gallery_category_model->get_video_albums_by_lang($data['category']->lang_id);

        $this->load->view('admin/includes/_header', $data);
        $this->load->view('admin/video/update_video_category', $data);
        $this->load->view('admin/includes/_footer');
    }
    ...
    public function update_gallery_category_post()
    {
     ...
     public function update_audio_category_post()
    {
        check_permission('audio');
        //validate inputs
        $this->form_validation->set_rules('name', trans("category_name"), 'required|xss_clean|max_length[200]');
        ...
        if ($this->form_validation->run() === false) {
            ...
            //category id
            $id = $this->input->post('id', true);
            if ($this->gallery_category_model->update_audio_cat($id)) {
                $this->session->set_flashdata('success', trans("category") . " " . trans("msg_suc_updated"));
                redirect(admin_url() . 'audio-sub-categories');
            } else {
                $this->session->set_flashdata('form_data', $this->gallery_category_model->add_audio_cat());
                $this->session->set_flashdata('error', trans("msg_error"));
                redirect($this->agent->referrer());
            }
        }
     ...
     public function update_video_category_post()
    {
        check_permission('video');
        //validate inputs
        $this->form_validation->set_rules('name', trans("category_name"), 'required|xss_clean|max_length[200]');
        ...
        if ($this->form_validation->run() === false) {
            ...
            //category id
            $id = $this->input->post('id', true);
            if ($this->gallery_category_model->update_video_cat($id)) {
                $this->session->set_flashdata('success', trans("category") . " " . trans("msg_suc_updated"));
                redirect(admin_url() . 'video-categories');
            } else {
            $this->session->set_flashdata('form_data', $this->gallery_category_model->add_audio_cat_video());
                $this->session->set_flashdata('error', trans("msg_error"));
                redirect($this->agent->referrer());
            }
        }
    ...
    public function delete_gallery_category_post()
    {
        ...
        public function delete_audio_category_post()
    {
        if (!check_user_permission('audio')) {
            exit();
        }
        $id = $this->input->post('id', true);
        //check if category has posts
    ...
    public function video_categories_by_album()
    {
    ...
    public function audio_categories_by_album()
    {
      $category_id = $this->input->post('cate_id', true);
      
        if (!empty($category_id)):
            $categories = $this->gallery_category_model->get_categories_by_audio_album($category_id);
            foreach ($categories as $item) {
                echo '<option value="' . $item->id . '">' . $item->name . '</option>';
            }
        endif;
