...
class Ajax_pagination{
	...
	var $last_link		= 'Last';
	var $uri_segment	= 3;
	var $full_tag_open	= '<div class="col-md-12 pagination" style="margin-left">';
	...
	function create_links(){
		...
		//if ($CI->uri->segment($this->uri_segment) != 0){
			$this->cur_page =$this->page_no;

			//$curr_offset = $this->page_no;
	   ...
	   // $this->cur_page = floor(($this->cur_page/$this->per_page) + 1);
		...
		// SHOWING LINKS records of data
		if ($this->show_count){

			$curr_offset = $this->page_no;
			$info = 'Showing ' . ( 15*($curr_offset-1) + 1 ) . ' to ' ;
			
			if( ( $curr_offset*$this->per_page ) < ( $this->total_rows ) )
			$info .= $curr_offset*$this->per_page;
			else
			$info .= $this->total_rows;			
			$info .= ' of ' . $this->total_rows . ' records | ';			
			$output .= $info;
		}
		...
		if  ($this->cur_page > $this->num_links){
			$output .= $this->first_tag_open 
					. $this->getAJAXlink( $i , $this->first_link)
					. $this->first_tag_close; 
		}

		// Render the "previous" link
		if  ($this->cur_page != 1){
			$i = ($this->cur_page-2)*$this->per_page;
			if ($i == 0) $i = '';
		...
		// Write the digit links for all pages
		...
		if ($this->cur_page < $num_pages){
			if($this->cur_page==1){
				...
				. $this->getAJAXlink(15 , $this->next_link )
				. $this->next_tag_close;
			}
			else{
			$output .= $this->next_tag_open 
				. $this->getAJAXlink( 15*$this->cur_page , $this->next_link )
				. $this->next_tag_close;
			}
		}
	...
	function getAJAXlink($count, $text){
		...
		return '<a href="javascript:void(0);"' . $this->anchor_class . ' onclick="'.$this->link_func.'('.$count.')">'. $text .'</a>';
	}
