...
<div class="row table-filter-container">
    <div class="col-sm-12">
        <?php echo form_open(admin_url() . 'infographic', ['method' => 'GET']); ?>
        ...
        <div class="item-table-filter">
            ...
            <select id="infographic_category" name="infographic_category" class="form-control">
                <option value=""><?php echo trans("select"); ?></option>
                <?php if (!empty($this->input->get('lang_id', true))) {
                    $albums = $this->gallery_category_model->get_infographic_by_lang($this->input->get('lang_id', true));
                } else {
                    $albums = $this->gallery_category_model->get_infographic_by_lang($this->selected_lang->id);
                } ?>
                <?php
                if (!empty($albums)):
                    foreach ($albums as $item): ?>
                        <option value="<?php echo $item->id; ?>" <?php echo ($this->input->get('infographic_category', true) == $item->id) ? 'selected' : ''; ?>>
