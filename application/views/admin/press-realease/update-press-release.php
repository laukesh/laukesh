...
<?php
        ...
        <div class="col-sm-3">
            <div class="boxWidth">
        <img style="height: 200px;width: 200px;" src="<?php echo base_url() . html_escape($items->media_path); ?>" alt="" class="thumbnail img-responsive" >
    </div>
    ...
    <input type = "hidden" name="<?= 'img'.$i; ?>" value="<?php echo $items->media_path;?>" />
    <input type = "hidden" name="image_caption_id[]" value="<?php echo $items->id; ?>" />
<div style="display: flex;
    align-items: baseline;"><input type="checkbox" name="progress[]" class="checkbox_data check_disabled" id="progress" value="<?= $i; ?>" data-value="1"<?php if($items->media_path == $view_press_release[0]->feature_image){ echo 'checked="checked"';}else{ echo 'disabled'; }?> tabindex="1" onclick="ckChange(this,'1')">
    <label style="padding:6px;">MainImage</label>
...
<a href="javascript:void(0);" onclick="remove_media('<?php echo $items->id.','.$items->release_id.','.$items->media_type.','.$items->lang_id.','.$items->media_format.','.$items->media_size.','.$items->media_path.','.$items->caption.','.$items->status.','.$items->schedule_for_publish.','.$items->created_at.','.$items->created_by;?>');" class="remove_button"><img src="<?php echo base_url(); ?>/assets/img/delete-image.png" width="35px" height="35px"/></a>
    ...
    ?>         
      ...
      <div class="row">
          <div class="col-sm-3">
                  <input type="file" id="upload_file" name="img[]" size="20" accept=".png, .jpg, .jpeg, .gif">
            </div>
            <div class="col-sm-8">
...
<hr style="width:100%;text-align:left;margin-left:0">
             <div class="form-group">
                ...
                ?>
                        ...
                        <div class="col-md-2 col-sm-2 col-xs-12 col-option">
                            <input type="radio" id="rb_is_active_2" name="status" value=" <?php if($view_press_release[0]->status == 6){ echo'10';}else{ echo'2';}?>" onclick="hideDiv()" <?php if($view_press_release[0]->status == 2){ echo "checked=checked";}  ?> >
              ...
              <?php }else  if($this->auth_user->role == 'hq admin' && $this->auth_user->pro_category_id = $pro['0']['id'] && $this->auth_user->permission = 2)
                {?>
                       ...
                       <div class="col-md-2 col-sm-2 col-xs-12 col-option">
                        ...
                        <div class="col-md-2 col-sm-2 col-xs-12 col-option">
                            ...
                            <label for="rb_is_active_2" class="cursor-pointer">Reject</label>
                        </div>
                         <div class="col-md-2 col-sm-2 col-xs-12 col-option">
                ...
                <?php 
                  ...
                  <div class="col-md-2 col-xs-12 col-option">
                      ...
                      <?php } else if($view_press_release[0]->status == 2){ ?>

                        <div class="col-md-2 col-xs-12 col-option">
                            <input type="radio" name="status" value="6" onclick="ShowwDiv()" <?php if($view_press_release[0]->status == 6){ echo "checked=checked";} ?>>
                            <label for="rb_is_active_2" class="cursor-pointer">Reject</label>
                        </div>

                        <?php }else if($view_press_release[0]->status == 6){?>
                          <div class="col-md-2 col-xs-12 col-option">
                            <input type="radio" name="status" value="6" onclick="ShowwDiv()" <?php if($view_press_release[0]->status == 6){ echo "checked=checked";}  ?>>
                            <label for="rb_is_active_2" class="cursor-pointer">Reject</label>
                        </div>
...
<?php if($view_press_release[0]->status == 6 && $this->auth_user->role != 'pro_admin'){?>
...
<div class="container" id="reject_text" style="display:none;">
          <div class="row">
          <div class='col-sm-2'>
          <div class="form-group">
          <label class="control-label"><?php echo trans('reason'); ?></label>
          </div>
          </div>
          <div class='col-sm-8'>
          <textarea row="5" cols="5" name="reason_for_rejection" class="form-control"></textarea>
          </div>
         </div>
    </div>
...
// function show hide div for reason for rejection div

  function ShowwDiv(){
    //alert('hi naazreen khan');
    document.getElementById("reject_text").style.display = "";
    $('#welcomeDiv').hide();
...
$(document).ready(function(){
    var maxField = 10; 
    var addButton = $('.add_button_image');
    var wrapper = $('.field_wrapper_image'); 
    var x = 1;
    <?php if(!empty($view_press_release_image)){ ?>
     var i = '<?php echo count($view_press_release_image)-1; ?>';
    <?php } else { ?>
    var i = 0;
<?php } ?>
    $(addButton).click(function(){
    ...
    var fieldHTML = '<div><div class="row" style="margin-bottom:4px;"><div class="col-md-3"><input type="file" id="upload_file"  name="image[]" accept=".png, .jpg, .jpeg, .gif" value=""/></div><div class="col-md-8"><div class="boxinl"><input type="checkbox" name="progress"  id="progress'+x+'" data-value="'+i+'" class="checkbox_data" value="'+i+'" tabIndex="1" onClick="ckChange2(this,'+x+')">  <label style="padding:6px;">MainImage</label><input type="text" name="caption-image-text[]" style="margin-left: 10px;" value="" placeholder="Brief Description" class="form-control"></div></div><a href="javascript:void(0);" class="remove_button" data-value="'+x+'"><img src="<?php echo base_url(); ?>/assets/img/delete-image.png" width="35px" height="35px"/></a></div></div>'; 
    ...
    //Once remove button is clicked
    $(wrapper).on('click', '.remove_button', function(e){
     //var inputId= $(this).attr('data-value'));
     $(".checkbox_data").removeAttr('disabled',false);
    //$("$progress"+inputId).removeAttr();
...
function ckChange(ckType,ckid){
  ...
  var ckval2  = $('#progress').attr('data-value');
  $('#img_setup').val(ckval);
...
function ckChange2(ckType,ckid){
 
  var ckName = document.getElementsByName(ckType.name);
  var ckval  = $('#progress'+ckid).attr('data-value');
  var ckval2  = $('#progress').attr('data-value');
  if(ckval2){
    $(".check_disabled").attr('disabled','disabled');
  }
  $('#img_setup').val(ckval);
  for (var i = 0; i < ckName.length; i++) {
    if (!ckType.checked){
