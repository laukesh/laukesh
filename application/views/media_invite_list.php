...
function getData(page){ 
  ...
  $('body').prepend("<div class='loader_css' style='display:block'><img class='loader_img_css' src='<?php echo base_url();?>assets-front/images/loading.gif'/><span class='loader_text_css'>Loading ...</span></div>");
    $.ajax({
        method: "POST",
        url: "<?php echo base_url();?>home_controller/media_invite_list_ajax",
        data: {page: page,
        ...
        // alert(serialnumber);
         ...
         //alert(datafinal);
          ...
          $("#error_msg").html('<div class="danger_message col-md-4" style="background-color: #ffdddd; margin-bottom: 40px;padding: 10px 12px; margin-top:40px; text-align:center; left: 356px"><p><strong><?php echo $tran_lang['data_not_found'];?></strong></p></div>');
           ...
           $('#msg_error').hide();
             ...
             var serialnumber = (serialnumber-1)*15;
              ...
              for ( var i in datafinal) {
                ...
                $("#media_list").html('<div class="danger_message col-md-4 error_style_center"><p><strong><?php echo $tran_lang['data_not_found'];?></strong></p></div>');
...
<a href="<?php //echo base_url('media-invite-archive');?>" type="button" class="archived1"><?php //echo $tran_lang['archives'];?></a>
 ...
 <div class="col-md-12" ><h1 class="media-heading"><?php echo $tran_lang['media_invite'];?><br><span class="media-heading-sub"><a href="<?php echo base_url('media-invite-archive');?>"><?php echo $tran_lang['fifteen_days_media_invite'];?></a></span></h1>
 ...
 <table class="table table-bordered tablefont" id="error_msg">
  <thead class="table-dark1">
    <tr> 
      ...
      <th scope="col" class=""><?php echo $tran_lang['contact_details'];?></th>
      <th scope="col" rowspan ="2" class=""><?php echo $tran_lang['remarks'];?></th>
    </tr>
  ...
  <tbody id="media_list">
      ...
      foreach($media_invite as $item){
        $i++;
        $d = date_create($item->date_of_event);
      ?>
      <tr> 
      <td class="left"><?php echo $i;?></td>     
      <td class="left"><?php echo $item->name;?></td>
      <td class="left"><a href="<?php echo base_url('media-invite-detail/').$item->id ;?>"><?php echo $item->title;?></a></td>
      <td class="left"><?php echo $item->venue_event;?></td>
      <td class="left"><?php echo $d->format('d/m/Y H:i A'); ?></td>
      <td class="left Cont_details">
        <strong>Person name.</strong><?php echo $item->firstname." ".$item->lastname;?>
        <strong>Mob.</strong>+<?php echo $item->mobile;?></td>
...
<div id="msg_error" class="danger_message col-md-4" style="background-color: #ffdddd; 
